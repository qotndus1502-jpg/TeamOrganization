...
interface Team {
  ...
  locationId: number;
  location: { id: number; company: string; name: string; type: string };
}
...
export default function AdminCategoriesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/admin/categories", {
      ...
      body: JSON.stringify({ name: newCategory.trim(), company: selCompany }),
    });
  ...
  const handleRename = async (cat: Category) => {
    ...
    // 팀들의 category도 업데이트
    const targetTeams = teams.filter((t) => t.location.company === cat.company && t.category === cat.name);
    ...
    // Category 자체도 삭제 후 재생성 (unique constraint)
    ...
    await fetch("/api/admin/categories", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: editValue.trim(), company: cat.company }),
    });
    setEditingId(null);
    load();
  };
  ...
  const grouped = COMPANIES.map((company) => ({
    company,
    cats: categories.filter((c) => c.company === company),
      ...
      <form onSubmit={handleAdd} className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <div className="flex gap-3 items-end">
          <div className="w-40">
            ...
            <select required value={selCompany} onChange={(e) => setSelCompany(e.target.value)}
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">본부명</label>
        ...
        <div key={g.company} className="mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {g.cats.map((cat) => {
                  const catTeams = teams.filter((t) => t.location.company === cat.company && t.category === cat.name);
                  return (
