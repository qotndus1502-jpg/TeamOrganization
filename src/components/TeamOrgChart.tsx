            ...
            <div className="grid grid-cols-2 gap-10">
              ...
              <div>
                <div className="flex items-center justify-between mb-5 pb-2 border-b border-border">
                  ...
                  {canEdit && <SectionEditBtn onClick={() => { const rd = employee.resumeData ? JSON.parse(employee.resumeData) : {}; const exp = (rd.experience || []).map((e: Record<string, string>) => { const desc = e.description || e.task || ""; return { company: e.company || "", position: e.position || "", period: e.period || "", startDate: e.startDate || "", endDate: e.endDate || "", task: e.task || "", descItems: desc.split("\n").filter(Boolean).length > 0 ? desc.split("\n").filter(Boolean) : [""] }; }); setExtCareerItems(exp.length > 0 ? exp : [{ company: "", position: "", period: "", startDate: "", endDate: "", task: "", descItems: [""] }]); setEditSection("extCareer"); }} />}
                </div>
                <div className="space-y-3">
